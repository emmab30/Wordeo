...
BotUser.generateRandomRoom = (socketHandler) => {
    ...
    dataSource.query(query, (err1, bots) => {
        if(bots) {
            ...
            socketHandler.app.models.Room.find({ where : { deletedAt: null } }, (err, rooms) => {

                //Percentages for multiplier exp rooms
                let multiplierExp = 1;
                if(!_.some(rooms, _.conforms({ 'multiplierExp' : (e) => { return e > 1 } }))) {
                    let randomNumber = _.random(0, 100);
                    if(randomNumber < 2) { //2% of creating rooms x5
                        multiplierExp = 5;
                    } else if(randomNumber <= 5) { //7% of creating rooms x4
                        multiplierExp = 4;
                    } else if(randomNumber <= 10) { //15% of creating rooms x3
                        multiplierExp = 3;
                    } else if(randomNumber <= 20) { //35% of creating rooms x2
                        multiplierExp = 2;
                    }
                }
...
BotUser.stopSimulatingStats = function(socketHandler, roomId) {
    let stoppedBots = [];
